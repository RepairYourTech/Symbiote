...
const TestComponent = () => {
	const { allowedCommands, setAllowedCommands, soundEnabled, showSymbioteIgnoredFiles, setShowSymbioteIgnoredFiles } =
		useExtensionState()
	return (
		<div>
			<div data-testid="allowed-commands">{JSON.stringify(allowedCommands)}</div>
			<div data-testid="sound-enabled">{JSON.stringify(soundEnabled)}</div>
			<div data-testid="show-symbiote-ignored-files">{JSON.stringify(showSymbioteIgnoredFiles)}</div>
			<button data-testid="update-button" onClick={() => setAllowedCommands(["npm install", "git status"])}>
				Update Commands
			</button>
			<button data-testid="toggle-symbiote-ignore-button" onClick={() => setShowSymbioteIgnoredFiles(!showSymbioteIgnoredFiles)}>
				Update Commands
			</button>
		</div>
	)
...
describe("ExtensionStateContext", () => {
	...
	it("initializes with showSymbioteIgnoredFiles set to true", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		expect(JSON.parse(screen.getByTestId("show-symbiote-ignored-files").textContent!)).toBe(true)
	})

	it("updates showSymbioteIgnoredFiles through setShowSymbioteIgnoredFiles", () => {
		render(
			<ExtensionStateContextProvider>
				<TestComponent />
			</ExtensionStateContextProvider>,
		...
		act(() => {
			screen.getByTestId("toggle-symbiote-ignore-button").click()
		})

		expect(JSON.parse(screen.getByTestId("show-symbiote-ignored-files").textContent!)).toBe(false)
	})
