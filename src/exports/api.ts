...
import { SymbioteSettings, SymbioteEvents, SymbioteEventName, ClineMessage } from "../schemas"
import { IpcOrigin, IpcMessageType, TaskCommandName, TaskEvent } from "../schemas/ipc"

import { SymbioteAPI } from "./interface"
import { IpcServer } from "./ipc"
import { outputChannelLog } from "./log"

export class API extends EventEmitter<SymbioteEvents> implements SymbioteAPI {
			...
			ipc.on(IpcMessageType.TaskCommand, async (_clientId, { commandName, data }) => {
				switch (commandName) {
					case TaskCommandName.StartNewTask:
						...
						await this.startNewTask(data as any)
						break
	...
	public override emit<K extends keyof SymbioteEvents>(
		eventName: K,
		...args: K extends keyof SymbioteEvents ? SymbioteEvents[K] : never
	) {
		const data = { eventName: eventName as SymbioteEventName, payload: args } as TaskEvent
	...
	public async startNewTask({
		...
		if (newTab) {
			...
			await vscode.commands.executeCommand("symbiote.SidebarProvider.focus")
	...
	public async setConfiguration(values: SymbioteSettings) {
	...
	public async createProfile(name: string) {
		...
		if (profiles.some((profile: { name: string }) => profile.name === name)) {
	...
	public getProfiles() {
		return (this.getConfiguration().listApiConfigMeta || []).map((profile: { name: string }) => profile.name)
	}
	...
	public async deleteProfile(name: string) {
		const currentSettings = this.getConfiguration()
		const profiles = currentSettings.listApiConfigMeta || []
		const targetIndex = profiles.findIndex((p: { name: string }) => p.name === name)
		...
		const newSettings: SymbioteSettings = {
			...currentSettings,
	...
	private registerListeners(provider: ClineProvider) {
		provider.on("clineCreated", (cline) => {
			cline.on("taskStarted", async () => {
				this.emit(SymbioteEventName.TaskStarted, cline.taskId)
			...
			cline.on("message", async (message) => {
				this.emit(SymbioteEventName.Message, { taskId: cline.taskId, ...message })
			...
			cline.on("taskModeSwitched", (taskId, mode) => this.emit(SymbioteEventName.TaskModeSwitched, taskId, mode))

			cline.on("taskTokenUsageUpdated", (_, usage) =>
				this.emit(SymbioteEventName.TaskTokenUsageUpdated, cline.taskId, usage),
			)

			cline.on("taskAskResponded", () => this.emit(SymbioteEventName.TaskAskResponded, cline.taskId))

			cline.on("taskAborted", () => {
				this.emit(SymbioteEventName.TaskAborted, cline.taskId)
				this.taskMap.delete(cline.taskId)
			})

			cline.on("taskCompleted", async (_, tokenUsage, toolUsage) => {
				this.emit(SymbioteEventName.TaskCompleted, cline.taskId, tokenUsage, toolUsage)
				this.taskMap.delete(cline.taskId)
			...
			cline.on("taskSpawned", (childTaskId) => this.emit(SymbioteEventName.TaskSpawned, cline.taskId, childTaskId))
			cline.on("taskPaused", () => this.emit(SymbioteEventName.TaskPaused, cline.taskId))
			cline.on("taskUnpaused", () => this.emit(SymbioteEventName.TaskUnpaused, cline.taskId))

			this.emit(SymbioteEventName.TaskCreated, cline.taskId)
		})
	}
