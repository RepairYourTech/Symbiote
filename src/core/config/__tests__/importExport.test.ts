	...
	describe("exportSettings", () => {
		...
		it("should export settings to the selected file location", async () => {
			...
			;(vscode.window.showSaveDialog as jest.Mock).mockResolvedValue({
				fsPath: "/mock/path/symbiote-settings.json",
			})
			...
			expect(fs.writeFile).toHaveBeenCalledWith(
				"/mock/path/symbiote-settings.json",
				JSON.stringify(
					{
		...
		it("should use the correct default save location", async () => {
			...
			expect(vscode.Uri.file).toHaveBeenCalledWith(path.join("/mock/home", "Documents", "symbiote-settings.json"))
		})
	})
})
