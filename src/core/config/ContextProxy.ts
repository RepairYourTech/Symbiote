...
type SymbioteSettingsKey = keyof SymbioteSettings
...
export class ContextProxy {
	...
	public setValue<K extends SymbioteSettingsKey>(key: K, value: SymbioteSettings[K]) {
		return isSecretStateKey(key as string) ? this.storeSecret(key as SecretStateKey, value as string) : this.updateGlobalState(key as GlobalStateKey, value)
	}

	public getValue<K extends SymbioteSettingsKey>(key: K): SymbioteSettings[K] {
		return isSecretStateKey(key as string)
			? (this.getSecret(key as SecretStateKey) as SymbioteSettings[K])
			: (this.getGlobalState(key as GlobalStateKey) as SymbioteSettings[K])
	}

	public getValues(): SymbioteSettings {
		return { ...this.getAllGlobalState(), ...this.getAllSecretState() }
	}

	public async setValues(values: SymbioteSettings) {
		const entries = Object.entries(values) as [SymbioteSettingsKey, unknown][]
