...
// If a user modifies a file outside of Symbiote, the context may become stale and need to be updated.
// We do not want Symbiote to reload the context every time a file is modified, so we use this class merely
// to inform Symbiote that the change has occurred, and tell Symbiote to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Symbiote was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to Symbiote via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of Symbiote, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	private recentlyEditedBySymbiote = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.onDidChange(() => {
			if (this.recentlyEditedBySymbiote.has(filePath)) {
				this.recentlyEditedBySymbiote.delete(filePath) // This was an edit by Symbiote, no need to inform Symbiote
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Symbiote
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to Symbiote via a tool, mention, or edit.
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				Symbiote_read_date: getLatestDateForField(filePath, "Symbiote_read_date"),
				Symbiote_edit_date: getLatestDateForField(filePath, "Symbiote_edit_date"),
			...
			switch (source) {
				...
				// Symbiote_edited: Symbiote has edited the file
				case "Symbiote_edited":
					newEntry.Symbiote_read_date = now
					newEntry.Symbiote_edit_date = now
					this.checkpointPossibleFiles.add(filePath)
					break

				// read_tool/file_mentioned: Symbiote has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.Symbiote_read_date = now
					break
	...
	// Marks a file as edited by Symbiote to prevent false positives in file watchers
	markFileAsEditedBySymbiote(filePath: string): void {
		this.recentlyEditedBySymbiote.add(filePath)
	}
