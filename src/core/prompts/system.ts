...
async function generatePrompt(
	...
	language?: string,
	symbioteIgnoreInstructions?: string,
): Promise<string> {
	if (!context) {
...
${await addCustomInstructions(promptComponent?.customInstructions || modeConfig.customInstructions || "", globalCustomInstructions || "", cwd, mode, { language: language ?? formatLanguage(vscode.env.language), symbioteIgnoreInstructions })}`
...
export const SYSTEM_PROMPT = async (
	...
	if (fileCustomSystemPrompt) {
		...
		const customInstructions = await addCustomInstructions(
			...
			cwd,
			mode,
			{ language: language ?? formatLanguage(vscode.env.language), symbioteIgnoreInstructions },
		)
