...
describe("appendToFileTool", () => {
	...
	beforeEach(() => {
		...
		const mockClineBase = {
			...
			} as unknown as SymbioteIgnoreController,
			api: {
	...
	describe("Error handling", () => {
		...
		it("should handle symbiote-ignore validation failures", async () => {
			...
			// Verify
			expect(mockCline.say).toHaveBeenCalledWith("symbiote_ignore_error", "test.txt")
			expect(formatResponse.symbioteIgnoreError).toHaveBeenCalledWith("test.txt")
