...
export async function executeCommandTool(
	...
	try {
		if (block.partial) {
			...
			const ignoredFileAttemptedToAccess = cline.symbioteIgnoreController?.validateCommand(command)

			if (ignoredFileAttemptedToAccess) {
				await cline.say("symbiote_ignore_error", ignoredFileAttemptedToAccess)
				pushToolResult(formatResponse.toolError(formatResponse.symbioteIgnoreError(ignoredFileAttemptedToAccess)))
				return
			}
