...
export async function readFileTool(
	...
	try {
		if (block.partial) {
			...
			if (!accessAllowed) {
				await cline.say("symbiote_ignore_error", relPath)
				const errorMsg = formatResponse.symbioteIgnoreError(relPath)
			...
			if (isRangeRead) {
				...
				const res = await Promise.all([
					...
					parseSourceCodeDefinitionsForFile(absolutePath, cline.symbioteIgnoreController),
				])
