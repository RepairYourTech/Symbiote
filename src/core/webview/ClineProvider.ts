...
import { GlobalState, ProviderSettings, SymbioteSettings } from "../../schemas"
import { t } from "../../i18n"
...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "symbiote.SidebarProvider" // used in package.json as the view's id
	public static readonly tabPanelId = "symbiote.TabPanelProvider"
	...
	*/
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\Symbiote\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "Symbiote", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/Symbiote/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "Symbiote", "MCP")
		} else {
			// Linux: ~/.local/share/Symbiote/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "Symbiote", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".symbiote", "mcp")
		}
	...
	async getStateToPostToWebview() {
		...
		const allowedCommands = vscode.workspace.getConfiguration("symbiote").get<string[]>("allowedCommands") || []
		const cwd = this.cwd
		...
		return {
			...
			telemetryKey,
			machineId,
			showSymbioteIgnoredFiles: showSymbioteIgnoredFiles ?? true,
			language,
	...
	async getState() {
		...
		return {
			...
			showSymbioteIgnoredFiles: stateValues.showSymbioteIgnoredFiles ?? true,
	...
	public async setValue<K extends keyof SymbioteSettings>(key: K, value: SymbioteSettings[K]) {
		await this.contextProxy.setValue(key, value)
	}

	public getValue<K extends keyof SymbioteSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: SymbioteSettings) {
