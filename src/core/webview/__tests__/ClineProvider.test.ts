...
describe("ClineProvider", () => {
	...
	test("handles showSymbioteIgnoredFiles setting", async () => {
		await provider.resolveWebviewView(mockWebviewView)
		const messageHandler = (mockWebviewView.webview.onDidReceiveMessage as jest.Mock).mock.calls[0][0]

		// Default value should be true
		expect((await provider.getState()).showSymbioteIgnoredFiles).toBe(true)

		// Test showSymbioteIgnoredFiles with true
		await messageHandler({ type: "showSymbioteIgnoredFiles", bool: true })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showSymbioteIgnoredFiles", true)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showSymbioteIgnoredFiles).toBe(true)

		// Test showSymbioteIgnoredFiles with false
		await messageHandler({ type: "showSymbioteIgnoredFiles", bool: false })
		expect(mockContext.globalState.update).toHaveBeenCalledWith("showSymbioteIgnoredFiles", false)
		expect(mockPostMessage).toHaveBeenCalled()
		expect((await provider.getState()).showSymbioteIgnoredFiles).toBe(false)
	})
...
// Telemetry tests removed as telemetry functionality has been removed from Symbiote
