...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "language":
			...
			await provider.postStateToWebview()
			break
		case "showSymbioteIgnoredFiles":
			await updateGlobalState("showSymbioteIgnoredFiles", message.bool ?? true)
			await provider.postStateToWebview()
			break
...
const generateSystemPrompt = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	const symbioteIgnoreInstructions = provider.getCurrentCline()?.symbioteIgnoreController?.getInstructions()
