...
import { SymbioteIgnoreController } from "../../core/ignore/SymbioteIgnoreController"
...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (symbioteIgnoreController && !symbioteIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, symbioteIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = symbioteIgnoreController ? symbioteIgnoreController.filterPaths(filesToParse) : filesToParse
	...
	for (const file of markdownFiles) {
		// Check if we have permission to access this file
		if (symbioteIgnoreController && !symbioteIgnoreController.validateAccess(file)) {
			continue
		}
	...
	for (const file of otherFiles) {
		const definitions = await parseFile(file, languageParsers, symbioteIgnoreController)
		if (definitions) {
			result += `# ${path.relative(dirPath, file).toPosix()}\n${definitions}\n`
		}
	}
 ...
 * @param symbioteIgnoreController - Optional controller to check file access permissions
