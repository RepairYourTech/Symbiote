...
const initWorkspaceRepo = async ({
	workspaceDir,
	userName = "Symbiote",
	userEmail = "support@symbiote.dev",
	...
	(klass, prefix) => {
		...
		describe(`${klass.name}#renameNestedGitRepos`, () => {
			it("handles nested git repositories during initialization", async () => {
				...
				await mainGit.init()
				await mainGit.addConfig("user.name", "Symbiote")
				await mainGit.addConfig("user.email", "support@symbiote.dev")
				...
				await nestedGit.addConfig("user.name", "Symbiote")
				await nestedGit.addConfig("user.email", "support@symbiote.dev")
