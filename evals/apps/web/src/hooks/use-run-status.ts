...
import { TokenUsage, taskEventSchema, SymbioteEventName, EvalEventName } from "@evals/types"
...
export const useRunStatus = (run: Run) => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case SymbioteEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case SymbioteEventName.TaskTokenUsageUpdated: {
