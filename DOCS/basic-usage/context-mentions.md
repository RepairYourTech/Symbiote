...
Context mentions are a powerful way to provide Symbiote with specific information about your project, allowing it to perform tasks more accurately and efficiently. You can use mentions to refer to files, folders, problems, and Git commits. Context mentions start with the `@` symbol.
...
|----------|-------------|
| **`.symbiote-ignore` bypass** | File and folder `@mentions` bypass `.symbiote-ignore` checks when fetching content for context. Content from ignored files will be included if directly mentioned. |
